          ...
          {offers.map((offer) => {
            ...
            return (
              <article className={styles.offerCard} key={offer.id}>
                <header><span className={`${styles.lifecycle} ${styles[offer.status]}`}>{offer.status === "active" ? "Live" : offer.status.charAt(0).toUpperCase() + offer.status.slice(1)}</span><span><Broadcast aria-hidden size={14} /> {publishingLabel[offer.status]}</span></header>

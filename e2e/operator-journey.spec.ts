...
test("operator reviews setup state and changes theme without fabricated actions", async ({ page }) => {
  ...
  await expect(page.getByText("Discord bot").locator("..")).toContainText("Discord bot is not connected");
  ...
  await expect(page.getByText("Database").locator("../..")).toContainText("Not connected");
